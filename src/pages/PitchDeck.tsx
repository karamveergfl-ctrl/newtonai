...
import {
  ...
  BookCheck, Flag, Play, ImageIcon, Calculator, Loader2 } from
"lucide-react";
      ...
      <div className="max-w-5xl mx-auto w-full">
        ...
        <div className="mt-6 bg-red-500/10 border border-red-500/20 rounded-xl p-4 animate-fade-in" style={{ animationDelay: "700ms" }}>
          <p className="text-red-300 text-sm font-medium text-center">
            Result: ₹5–15 lakh spent on smart boards that function as ₹5000 projectors
          </p>
        </div>
      </div>
    </div>);
...
export default function PitchDeck() {
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      if (e.key === "ArrowDown" || e.key === "ArrowRight" || e.key === " ") {e.preventDefault();next();} else
      if (e.key === "ArrowUp" || e.key === "ArrowLeft") {e.preventDefault();prev();} else

...
serve(async (req) => {
  ...
  try {
    ...
    if (!LOVABLE_API_KEY || !HAS_YT_KEY) throw new Error("API keys not configured");
    ...
    const topicsWithVideos = await Promise.all(topics.map(async (topic: any) => {
      const ytRes = await fetchYouTube(`https://www.googleapis.com/youtube/v3/search?part=snippet&maxResults=3&q=${encodeURIComponent(topic.heading + " educational")}&type=video`);
